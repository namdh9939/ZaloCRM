<template>
  ...
  <div v-else class="chat-container d-flex" style="height: calc(100vh - 64px);">
    ...
    <MessageThread
      ...
      @send="sendMessage"
      @send-attachments="onSendAttachments"
...
const {
  ...
  fetchConversations, fetchAiConfig, selectConversation, sendMessage, sendAttachments,
...
async function onSendAttachments(payload: { files: File[]; caption: string }) {
  await sendAttachments(payload.files, payload.caption);
}
